...
if (!function_exists('__')) {
    ...
    $translations = [
        ...
        'hi' => [
            ...
            'Client' => 'क्लाइंट',
            'Secure Encrypted Chat' => 'सुरक्षित एन्क्रिप्टेड चैट'
        ]
    ];
    function __($key) {
...
if ($session_id > 0) {
    $stmt = $conn->prepare("
        SELECT cs.status, cs.start_time, u.name AS client_name, u.gender 
        FROM chat_sessions cs 
        JOIN users u ON cs.user_id = u.id 
        WHERE cs.id = ? AND cs.astrologer_id = ?
    ");
    ...
    if ($chat_data && $chat_data['status'] === 'active') {
        $is_active_chat = true;
        $start_timestamp = strtotime($chat_data['start_time']);
...
<head>
    ...
    <title><?= $is_active_chat ? __('Live Chat') : 'No Active Chats' ?> - Astrologer Panel</title>
...
<?php if (!$is_active_chat): ?>
    ...
    <!-- Astrologer Header Included -->
    <?php include '../astrologer/header.php'; ?>
    ...
    <main class="flex-1 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p class="text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-8 leading-relaxed">
            It looks like you don't have any ongoing client sessions right now. Please return to your dashboard to monitor incoming requests.
        </p>
        
        <a href="/astrologer/dashboard.php" class="bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 text-white font-semibold py-3 px-8 rounded-full shadow-lg transition transform hover:-translate-y-0.5 active:scale-95">
            Return to Dashboard
        </a>
    </main>

    <!-- Astrologer Footer Included -->
    <?php include '../astrologer/footer_astrologer.php'; ?>
...
<?php else: ?>
    ...
    <div class="top-header bg-white shadow-sm px-4 py-3 flex justify-start items-center z-10 border-b border-gray-200">
        <div class="flex items-center gap-3">
            <a href="/astrologer/dashboard.php" class="w-10 h-10 rounded-full flex items-center justify-center text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 transition shadow-sm mr-1">
                <i class="fas fa-arrow-left"></i>
            </a>
            <div class="w-12 h-12 bg-blue-100 dark:bg-blue-900/40 rounded-full flex items-center justify-center text-blue-600 dark:text-blue-400 text-xl shadow-sm">
                <i class="fas fa-user"></i>
            </div>
            <div>
                <h2 class="font-bold text-gray-900 text-lg leading-tight">
                    <?= htmlspecialchars($chat_data['client_name'] ?: __('Client'), ENT_QUOTES, 'UTF-8') ?>
                </h2>
                <div class="flex items-center gap-1.5 text-xs font-bold text-emerald-500 tracking-wide mt-0.5">
                    <span class="w-2 h-2 rounded-full bg-emerald-500 timer-pulse"></span> 
    ...
    <div id="chatBox" class="chat-container flex flex-col pb-[10px]">
        <div class="text-center my-4">
            <span class="bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest">
                <?= __('Secure Encrypted Chat') ?>
            </span>
        </div>
    </div>
    ...
    <div id="inputAreaWrapper" class="input-area shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] bg-white dark:bg-gray-800 absolute bottom-0">
        ...
        <button id="keyboardToggleBtn" class="w-9 h-9 text-gray-500 dark:text-gray-400 rounded-full flex items-center justify-center shrink-0 active:bg-gray-100 dark:active:bg-gray-700 transition">
            <i id="kbToggleIcon" class="fas fa-keyboard text-lg transition-transform duration-200"></i>
        </button>
    ...
    // ==========================================
    // 60-SECOND INACTIVITY AUTO-END SYSTEM
    // ==========================================
    let chatInactivityTimer;
    const MAX_INACTIVITY_MS = 60000; // 60 seconds

    function triggerInactivityEnd() {
        setInterfaceLock(true); // Lock the UI
        clearInterval(checkInterval); // Stop polling
        // Redirect to the termination script to handle billing and closure
        window.location.replace(`/astrologer/end_chat.php?session_id=${sessionId}`);
    }

    function resetChatInactivity() {
        clearTimeout(chatInactivityTimer);
        chatInactivityTimer = setTimeout(triggerInactivityEnd, MAX_INACTIVITY_MS);
    }

    // Initialize timer on load
    resetChatInactivity();

    // Reset timer on Astrologer conversational activity
    msgInput.addEventListener('input', resetChatInactivity);
    keyboardRows.addEventListener('touchstart', resetChatInactivity, { passive: true });
    document.getElementById('galleryInput').addEventListener('change', resetChatInactivity);
    // ==========================================
    ...
    async function fetchMessages() {
        try {
            const res = await fetch(`/api/chat_handler.php?action=fetch&session_id=${sessionId}&last_id=${lastMsgId}`);
            const data = await res.json();
            ...
            if (data.status === 'success' && data.messages.length > 0) {
                ...
                data.messages.forEach(msg => {
                    const isMe = msg.sender_type === 'astrologer';
                ...
                // Reset inactivity timer ONLY if there are genuinely new messages in the conversation
                if (hasNewMessages) {
                    resetChatInactivity();
                    snapToBottom();
                }
                ...
                // Once the User ends the chat, this triggers and cleanly pushes the Astrologer out
                clearInterval(checkInterval);
                window.location.href = `/astrologer/dashboard.php`;
            }
        } catch (e) { console.error("Sync Error:", e); }
    }

    checkInterval = setInterval(fetchMessages, 1500);
    fetchMessages();
    </script>
<?php endif; ?>
