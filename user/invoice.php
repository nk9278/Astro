...
// Fetch specific order details, ENSURING it belongs to the logged-in user
$sql = "SELECT po.*, p.name as product_name, p.price as base_price, u.email as user_email 
        FROM product_orders po 
        JOIN products p ON po.product_id = p.id 
        JOIN users u ON po.user_id = u.id 
        WHERE po.id = ? AND po.user_id = ? LIMIT 1";

$stmt = $conn->prepare($sql);
$stmt->execute([$order_id, $user_id]);
$order = $stmt->fetch(PDO::FETCH_ASSOC);

// If no order is found, it either doesn't exist or belongs to someone else
if (!$order) {
...
<body onload="window.print()">
    ...
    <div class="max-w-[800px] mx-auto mt-8 flex justify-end px-4 no-print gap-4 mb-4">
        <a href="/user/orders.php" class="px-6 py-2 bg-gray-200 text-gray-800 font-bold rounded-lg shadow hover:bg-gray-300 transition-colors">
            Back to Orders
        </a>
    ...
    <div class="invoice-box border border-gray-100">
        ...
        <div class="flex justify-between items-start border-b border-gray-200 pb-8 mb-8">
            <div class="flex flex-col">
                <img src="/assets/images/FP%20Logo.png" alt="The Fortune Pathway" class="w-20 h-20 object-contain mb-2 rounded-full border border-gray-100 p-1 shadow-sm">
        ...
        <div class="flex justify-between mb-10">
            <div class="w-1/2 pr-4">
                <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Ship To / Billed To</p>
